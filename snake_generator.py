...
def simulate_snake(grid: list[list[int]], init_length: int = 4, grow_every: int = 3):
    """
    ...
    Grows +1 segment every `grow_every` eaten contribution dots.
    ...
    while foods and step < max_steps:
        ...
        if head in foods:
            ...
            eaten_count += 1
            will_grow = (eaten_count % grow_every == 0)
            if will_grow:
    ...
    print(f"Simulation completed: {len(chain)} frames, {len(eaten_times)}/{initial_food_count} contributions eaten, initial length: {init_length}, final length: {len(chain[-1])}")
    return chain, eaten_times, ncols, nrows


# ── Smooth CSS Keyframe SVG Rendering (Platane Style) ─────────────────────────
...
def build_svg(
    ...
    css_rules = [
        ...
        f'  --tail: {TAIL_CLR};',
        f'}}',
        f'.c {{ shape-rendering: geometricPrecision; width: {CELL}px; height: {CELL}px; }}',
        f'.s {{',
    ...
    # Grid tiles (clean contribution colors; transition to empty --c0 only when eaten)
    grid_elements = []
    for c in range(ncols):
        for r in range(nrows):
            ...
            if (c, r) in eaten_times:
                ...
                css_rules.append(
                    ...
                    f'0%, {p_eat - 0.001:.2f}% {{ fill: var(--c{lvl}); }} '
                    f'{p_eat + 0.001:.2f}%, 100% {{ fill: var(--c0); }} }}'
                )
                css_rules.append(f'.{anim_name} {{ fill: var(--c{lvl}); animation: {anim_name} {total_sec:.3f}s linear infinite; }}')
                grid_elements.append(f'  <rect class="c {anim_name}" x="{x}" y="{y}" rx="2" ry="2"/>')
            else:
                grid_elements.append(f'  <rect class="c" x="{x}" y="{y}" rx="2" ry="2" fill="var(--c{lvl})"/>')

    # Snake segments (separate moving elements on top of the grid)
    snake_elements = []
    for i in range(max_len):
        t_born = next((t for t in range(M) if len(chain[t]) > i), 0)
        x_birth, y_birth = chain[t_born][i]
        ...
        # Pause period at final position
        x_final, y_final = chain[M - 1][i]
        ...
        css_rules.append(f'.{anim_name} {{ animation-name: {anim_name}; transform: translate({PAD + chain[0][min(i, len(chain[0])-1)][0]*STRIDE}px, {PAD + chain[0][min(i, len(chain[0])-1)][1]*STRIDE}px); }}')

        extra_cls = ' s_tail' if i >= max_len - 2 else ''
        snake_elements.append(f'  <rect class="s s{i}{extra_cls}" rx="2" ry="2"/>')
...
def main():
    ...
    print("Running Snake AI simulation (eating all contributions, growing +1 every 3 dots)...")
    chain, eaten_times, ncols, nrows = simulate_snake(grid, init_length=4, grow_every=3)

    print("Generating Light Theme SVG (dist/github-contribution-grid-snake.svg)...")
    light_svg = build_svg(grid, chain, eaten_times, ncols, nrows, dark=False)
    ...
    dark_svg = build_svg(grid, chain, eaten_times, ncols, nrows, dark=True)
    with open("dist/github-contribution-grid-snake-dark.svg", "w", encoding="utf-8") as f:
        f.write(dark_svg)

    print("Success! Perfect Platane-style snake SVGs generated in dist/")
